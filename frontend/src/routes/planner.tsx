...
export const Route = createFileRoute("/planner")({
  ...
  component: () => <AppShell><Planner /></AppShell>,
});
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Brain-dump everything. Gemini extracts, prioritizes, and schedules.</p>
      </div>
      ...
      <GlassCard className="p-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-4">
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={listening ? undefined : startVoice} disabled={listening} className="glass border-white/10">
              {listening ? <><MicOff className="h-3.5 w-3.5 mr-1.5 text-rose-400 animate-pulse" /> Listening…</> : <><Mic className="h-3.5 w-3.5 mr-1.5" /> Voice</>}
            </Button>
            <Button variant="outline" size="sm" onClick={() => reprioMut.mutate()} disabled={reprioMut.isPending} className="glass border-white/10">
              <RefreshCw className={`h-3.5 w-3.5 mr-1.5 ${reprioMut.isPending ? "animate-spin" : ""}`} /> Reprioritize
            </Button>
          </div>
          <Button
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {EXAMPLES.map((ex) => (
            <button key={ex} onClick={() => setText(ex)} className="text-xs px-3 py-1.5 rounded-full glass border border-white/10 hover:bg-white/5">
      ...
      {planMut.isPending && (
        <GlassCard>
          ...
          <p className="text-xs text-muted-foreground mt-3">Gemini is reading your tasks, estimating time, and building your schedule…</p>
        </GlassCard>
      )}
        ...
        <GlassCard>
          ...
          <ul className="space-y-2">
            {planMut.data.tasks?.map((t: any, i: number) => (
              <motion.li key={t.id} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} transition={{delay:i*0.04}} className="glass rounded-xl p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="font-medium">{t.title}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">{relativeDeadline(t.deadline)} · {t.estimated_minutes}m · {t.category}</div>
                    {t.ai_reasoning && <div className="text-xs text-muted-foreground/80 mt-2 italic">"{t.ai_reasoning}"</div>}
                  </div>
      ...
      {planMut.isError && (
        <GlassCard className="border-rose-500/30">
          <div className="flex items-center gap-2 text-rose-300">
            <AlertTriangle className="h-4 w-4" /> {(planMut.error as any)?.message || "Something went wrong"}
          </div>
        </GlassCard>
      )}
    </div>
